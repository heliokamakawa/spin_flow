...
import 'package:spin_flow/controle/controlador_posicionamento_bikes.dart';
...
class _TelaPosicionamentoBikesState extends State<TelaPosicionamentoBikes> {
  final ControladorPosicionamentoBikes _controlador = ControladorPosicionamentoBikes();
  ...
  Future<void> _carregar() async {
    ...
    final salas = await _controlador.buscarTodasSalas();
    final bikes = await _controlador.buscarTodasBikes();
    final posicoes = await _controlador.buscarTodasPosicoes();
  ...
  Future<void> _abrirAtribuicao(int fila, int coluna) async {
    ...
    if (acao == 'remover') {
      await _controlador.excluirPosicaoPorPosicao(fila: fila, coluna: coluna);
      if (!mounted) return;
    ...
    if (acao == 'salvar' && bikeSelecionada != null) {
      await _controlador.salvarPosicao(
