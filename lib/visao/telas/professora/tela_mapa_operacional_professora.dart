...
import 'package:spin_flow/controle/controlador_mapa_operacional.dart';
...
class _TelaMapaOperacionalProfessoraState extends State<TelaMapaOperacionalProfessora> {
  final ControladorMapaOperacional _controlador = ControladorMapaOperacional();
  ...
  Future<void> _carregar() async {
    ...
    if (turma != null) {
      checkins = await _controlador.buscarCheckinsAtivosPorTurmaData(turmaId: turma.id ?? 0, data: _data);
    }
    ...
    if (turma != null) {
      final filaDTO = await _controlador.buscarFilaEsperaAtivosPorTurmaData(
        turmaId: turma.id ?? 0,
        data: _data,
      );
      for (int i = 0; i < filaDTO.length; i++) {
        final aluno = await _controlador.buscarAlunoPorId(filaDTO[i].alunoId);
  ...
  Future<void> _removerDaFila(_FilaEsperaItem item) async {
    ...
    await _controlador.sairDaFila(item.dto.id!);
    if (!mounted) return;
  ...
  Future<void> _cancelarCheckin(DTOCheckin c) async {
    ...
    await _controlador.cancelarCheckin(c.id!);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
