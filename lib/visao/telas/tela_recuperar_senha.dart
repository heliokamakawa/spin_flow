...
import 'package:spin_flow/controle/controlador_recuperar_senha.dart';
...
class _TelaRecuperarSenhaState extends State<TelaRecuperarSenha> {
  final ControladorRecuperarSenha _controlador = ControladorRecuperarSenha();
  ...
  Future<void> _verificarEmail() async {
    ...
    final usuario = await _controlador.buscarUsuarioPorEmailAtivo(email);
    final aluno = await _controlador.buscarAlunoPorEmailAtivo(email);
    if (!mounted) return;
  ...
  Future<void> _verificarIdentidade() async {
    ...
    if (_alunoId != null) {
      final aluno = await _controlador.buscarAlunoPorId(_alunoId!);
      if (!mounted) return;
  ...
  Future<void> _redefinirSenha() async {
    ...
    await _controlador.atualizarSenha(_usuarioId!, _novaSenhaController.text);
    if (!mounted) return;
