...
import 'package:spin_flow/controle/controlador_mapa_checkin.dart';
...
class _TelaMapaCheckinState extends State<TelaMapaCheckin> {
  final ControladorMapaCheckin _controlador = ControladorMapaCheckin();
  ...
  Future<void> _carregar() async {
    ...
    try {
      ...
      final checkins = await _controlador.buscarCheckinsAtivosPorTurmaData(
        turmaId: _turma!.id ?? 0,
        data: _data!,
      );
      final bikesBloqueadas = await _controlador.buscarBikeIdsEmManutencaoAtiva();
      final posicoesBloqueadas = await _controlador.buscarPosicoesPorBikeIds(bikesBloqueadas);
      final posicoesSala = (await _controlador.buscarTodasPosicoes()).where((p) => _estaNaGrade(p)).toList();
      final mix = await _controlador.buscarMixAtivoPorTurma(_turma!.id ?? 0, data: _data!);
      final professora = await _controlador.buscarPrimeiraProfessoraAtiva();
      final fila = await _controlador.buscarFilaAtivaPorTurmaData(turmaId: _turma!.id ?? 0, data: _data!);
  ...
  Future<DTOAluno?> _buscarAlunoLogado() async {
    ...
    return _controlador.buscarAlunoPorEmailAtivo(email);
  }
  ...
  Future<void> _reservar(int fila, int coluna) async {
    ...
    try {
      await _controlador.reservarComValidacao(dto);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _cancelarMeuCheckin() async {
    ...
    await _controlador.cancelarCheckin(_meuCheckin!.id!);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _entrarFilaDeEspera() async {
    ...
    try {
      await _controlador.entrarNaFila(
