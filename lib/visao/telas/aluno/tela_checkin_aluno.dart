...
import 'package:spin_flow/controle/controlador_checkin_aluno.dart';
...
class _TelaCheckinAlunoState extends State<TelaCheckinAluno> {
  final ControladorCheckinAluno _controlador = ControladorCheckinAluno();
  ...
  Future<void> _carregar() async {
    ...
    try {
      final turmas = await _controlador.buscarTurmasAtivas();
      ...
      final posicoesTodas = await _controlador.buscarTodasPosicoes();

      final List<_ResumoTurmaHoje> lista = [];
      for (final t in turmas) {
        if (!_diaCompativel(t, _hoje)) continue;

        final checkins = await _controlador.buscarCheckinsAtivosPorTurmaData(turmaId: t.id ?? 0, data: _hoje);
        final fila = await _controlador.buscarFilaAtivaPorTurmaData(turmaId: t.id ?? 0, data: _hoje);
  ...
  Future<void> _entrarFilaDeEspera(_ResumoTurmaHoje resumo) async {
    try {
      ...
      final jaTemCheckin = await _controlador.existeCheckinAtivoAluno(
        alunoId: aluno!.id!,
